package com.semivanilla.enderdragontweaks.loot;
    ...
    public static void bootStrap(File file) {
        ...
        for (String key : items.getKeys(false)) {
            try {
                ...
                if (itemName != null) meta.displayName(Util.parseMiniMessage(itemName));
                if (!lore.isEmpty()) {
                    List<Component> loreComponent = new ArrayList<>();
                    for (String s : lore) {
                        loreComponent.add(Util.parseMiniMessage(s));
                    }
